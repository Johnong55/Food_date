...
import { deleteCollection } from "@/services/saved/owned-data.service";
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (!id.success) {
      return apiError("INVALID_COLLECTION_ID", "ID bộ sưu tập không hợp lệ.", {
        status: 400,
        requestId,
      });
    }
    ...
    assertOwnedDataConfigured();
    await deleteCollection(id.data);
